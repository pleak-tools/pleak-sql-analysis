  ...
  if generateQueries args
    then do
      if policyAnalysis args
        then do
          (outputTableName,plcMaps,attMap,dataPath,initialQuery,initQueries,numOfoutputs,colNames,typeMap,taskMap,_,tableExprData,_,colTableCounts) <- PQ.getBanachAnalyserInput args (inputFp1 args) (inputFp2 args) (inputFp3 args) (inputFp4 args)
          performPolicyAnalysis args outputTableName dataPath (delimiter args) initialQuery initQueries numOfoutputs colNames typeMap taskMap tableExprData plcMaps attMap colTableCounts
        else do
