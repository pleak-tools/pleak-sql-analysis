...
-- TODO we have removed the scaling that depends on the number of rows, should implement it on Banach analyser side!!!
updateTableExpr :: B.TableExpr -> M.Map B.Var Double -> M.Map B.Var Double -> ADouble -> ADouble -> B.TableExpr
updateTableExpr expr mapCol mapLN queryAggrNorm dbAggrNorm =
    let n = 1.0 in
